...
def uti_plot_init(backend=DEFAULT_BACKEND, fname_format=None):
    ...
    if backend is not None:
        try:
            import uti_plot_matplotlib_cp
            import copy
            _bend = uti_plot_matplotlib_cp.Backend(backend, fname_format)
            _backend1 =copy.deepcopy(_bend)
            return
        except:
            traceback.print_exc()
    ...
    _backend1 = _BackendNone()

def uti_plot_show():
    """Display the plots"""
    #if '_backend' not in locals(): uti_plot_init() #?
    _backend1.uti_plot_show()
...
def uti_plot2d1d(ar2d, x_range, y_range, x=0, y=0, labels=('Horizontal position', 'Vertical position', 'Intensity'), units=None, graphs_joined=True):
    ...
    _backend1.uti_plot2d1d(ar2d, x_range, y_range, x, y, labels, graphs_joined)
...
def uti_data_file_plot(_fname, _read_labels=1, _e=0, _x=0, _y=0, _graphs_joined=True, _traj_report=False, _traj_axis='x'):
    ...
    """
    #if '_backend' not in locals(): uti_plot_init() #?
    _backend1.uti_data_file_plot(_fname, _read_labels, _e, _x, _y, _graphs_joined, _traj_report, _traj_axis)
...
class _BackendMissing(_BackendBase):
    def _backend_call(self, *args, **kwargs):
        ...
        func = getattr(_backend1, method_name)
        return func(*args)
